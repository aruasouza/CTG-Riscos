...
from setup import adlsFileSystemClient,records_path,ctg_temp_path
...
def find_datas(index):
    ...
    if risco in ['TRADING','MSO']:
        risco = 'INFLACAO'
    multithread.ADLDownloader(adlsFileSystemClient, lpath=file_path, 
    ...
    main_dataframe = pd.read_csv(file_path)
    os.remove(file_path)
