...
def success(past,future):
    return {'past':past.to_dict(orient="split"),'pred':future.to_dict(orient = 'split')},200
...
def predict_ipca(test = False):
    try:
        ...
        pred_df = pd.DataFrame({'prediction':prediction_final},index = list(map(lambda x: str(x),prediction_t.time_index.to_period('M'))))
        pred_df['std'] = std
        past_df = pd.DataFrame({'serie':ipca.values.ravel()},index = list(map(lambda x: str(x),ipca.index.to_period('M'))))
        # Salvando no Log
        return success(past_df,pred_df)
    except Exception as e:
        return error(e)
...
def predict_cambio(test = False):
    try:
        ...
        pred_df = pd.DataFrame({'prediction':prediction_final},index = list(map(lambda x: str(x),prediction_t.time_index.to_period('M'))))
        pred_df['std'] = std
        past_df = pd.DataFrame({'serie':cambio.values.ravel()},index = list(map(lambda x: str(x),cambio.index.to_period('M'))))
        # Salvando no Log
        return success(past_df,pred_df)
    except Exception as e:
        return error(e)
...
def predict_cdi(test = False):
    try:
        ...
        pred_df = pd.DataFrame({'prediction':prediction},
            index = list(map(lambda x: str(x),pd.period_range(start = cdi.index[-1] + relativedelta(months = 1),periods = len(prediction),freq = 'M'))))
        pred_df['std'] = std
        past_df = pd.DataFrame({'serie':cdi.values.ravel()},index = list(map(lambda x: str(x),cdi.index.to_period('M'))))
        # Salvando no Log
        return success(past_df,pred_df)
    except Exception as e:
        return error(e)
...
def predict_gsf(test = False):
    try:
        ...
        prediction = model.predict(12 * anos)
        pred_df = pd.DataFrame({'prediction':prediction},
            index = list(map(lambda x: str(x),pd.period_range(start = gsf.index[-1] + relativedelta(months = 1),periods = len(prediction),freq = 'M'))))
        pred_df['std'] = std
        past_df = pd.DataFrame({'serie':gsf.values.ravel()},index = list(map(lambda x: str(x),gsf.index.to_period('M'))))
        # Salvando no Log
        return success(past_df,pred_df)
    except Exception as e:
        return error(e)